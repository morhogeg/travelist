...
Need help getting API keys? Check the providers' documentation for Jira, OpenAI/OpenRouter, and Notion
...
Edit `config/principles.yaml` to match your strategy:
```yaml
principles:
  - name: "Customer First"
...
See `config/principles.yaml` for configuration template.
...
```bash
./jira-alignment/run-cli.sh --query "project = MOBILE AND created >= -7d"
```
...
| Command | Description |
|---------|-------------|
| `./jira-alignment/run-web.sh` | Start web dashboard |
| `./jira-alignment/run-cli.sh --test` | Run with sample data |
| `./jira-alignment/run-cli.sh --mode execution` | Full analysis mode |
| `./validate-setup.sh` | Check your setup |
| `./jira-alignment/steve-help.sh` | Show all commands |
...
- 🎯 View configuration templates in `config/` directory
- 💡 Learn about strategic principles in `config/principles.yaml`

## Get Help

- **Issues**: [GitHub Issues](https://github.com/morhogeg/STEVE/issues)
- **Documentation**: [README](./README.md) and [QUICKSTART](./QUICKSTART.md)
