...
// Ticket Card Component - Isolated to fix expansion issue
function TicketCard({ ticket, isExpanded, onToggle }: { ticket: any, isExpanded: boolean, onToggle: () => void }) {
  // Get category icon
  const getCategoryIcon = (category: string) => {
    switch(category) {
      case 'core_value': return <Target style={{ width: 16, height: 16 }} />;
      case 'strategic_enabler': return <Zap style={{ width: 16, height: 16 }} />;
      case 'drift': return <TrendingDown style={{ width: 16, height: 16 }} />;
      case 'distraction': return <AlertTriangle style={{ width: 16, height: 16 }} />;
      default: return null;
    }
  };
  ...
  return (
    ...
    >
      ...
      {(ticket.suggestedSummary || ticket.strategicComment) && (
        <button
        ...
        <div className="ticket-details">
          {ticket.strategicComment && (
            <div className="strategic-comment-section">
              ...
              <div className="strategic-comment-content">
                {ticket.strategicComment
                  .split('\n')
                  .filter((line: string) => !line.includes('Generated by STEVE') && !line.includes('Strategic Ticket Evaluation'))
                  .map((line: string, i: number) => {
                    ...
                    // Handle empty lines and dividers
                    if (!line.trim() || line.trim() === '⸻') {
                      return <div key={i} style={{ marginBottom: '0.5rem' }}></div>;
                    }
                    
                    // Render the line with HTML formatting
                    return (
                      <div 
                        key={i} 
...
function App() {
  ...
  const publishToNotion = async () => {
    ...
    try {
      const response = await fetch('http://localhost:8000/publish-to-notion', {
        method: 'POST',
        headers: {
            ...
            >
                    ...
                    <div key={category} className="category-section">
                      ...
                      <div className="tickets-grid">
                        {categoryTickets.map((ticket) => (
                          <div key={ticket.key} id={`ticket-${ticket.key}`}>
                            <TicketCard 
                              ticket={ticket}
                              isExpanded={expandedTickets.has(ticket.key)}
                              onToggle={() => toggleTicketExpansion(ticket.key)}
                            />
                          </div>
                        ))}
                      </div>
              ...
              {result.executiveSummary && (
                ...
                >
                  ...
                  {(() => {
                    // Track which lines have been consumed by other components
                    const consumedIndices = new Set<number>();
                    
                    return result.executiveSummary.split('\n').map((paragraph, index) => {
                      const trimmed = paragraph.trim();
                      
                      // Skip if this line was already consumed
                      if (consumedIndices.has(index)) {
                        console.log(`>>> SKIPPING index ${index} - already consumed by another component`);
                        return null;
                      }
                      
                      // Also skip if this is the intro text that follows health assessment (check previous non-empty lines)
                      if (index > 0) {
                        const lines = result.executiveSummary.split('\n');
                        // Look back up to 2 lines for health assessment
                        for (let i = index - 1; i >= Math.max(0, index - 2); i--) {
                          const prevLine = lines[i].trim().replace(/\*\*/g, '');
                          if (prevLine.includes('Strategic Health Assessment') && 
                              trimmed.replace(/\*\*/g, '').includes('current sprint') && 
                              trimmed.replace(/\*\*/g, '').includes('tickets')) {
                            console.log(`>>> SKIPPING index ${index} - this is the intro text that follows health assessment`);
                            return null;
                          }
                        }
                      }
                      ...
                      const cleanParagraph = trimmed.replace(/\*\*/g, '').trim();
                      const isBottomLine = cleanParagraph.toLowerCase().includes('bottom line');
                      const isHealthScore = cleanParagraph.includes('Strategic Health Assessment');
                      
                      // Debug logging
                      console.log(`\n=== Processing paragraph ${index} ===`);
                      console.log('Trimmed:', trimmed);
                      console.log('Clean:', cleanParagraph);
                      console.log('Is Health Score:', isHealthScore);
                      console.log('Is Bottom Line:', isBottomLine);
                      console.log('ConsumedIndices:', Array.from(consumedIndices));
                      
                      const isIntro = (index === 0 && !isHealthScore) || (index < 5 && !trimmed.includes(':') && !trimmed.startsWith('•') && !trimmed.startsWith('-') && !isHealthScore && !consumedIndices.has(index));
                      
                      // Handle Strategic Health Assessment FIRST before other checks
                      if (isHealthScore) {
                      console.log('>>> FOUND HEALTH ASSESSMENT at index', index);
                      const scoreMatch = cleanParagraph.match(/Strategic Health Assessment:?\s*(\d+)\/100/);
                      const score = scoreMatch ? scoreMatch[1] : '0';
                      const scoreValue = parseInt(score);
                      
                      // Get the next non-empty paragraph (intro text)
                      let introText = '';
                      const lines = result.executiveSummary.split('\n');
                      for (let i = index + 1; i < lines.length; i++) {
                        const nextLine = lines[i].trim();
                        if (nextLine) {
                          const nextLineClean = nextLine.replace(/\*\*/g, '');
                          console.log(`Found next non-empty line at index ${i}:`, nextLine);
                          console.log('Next line clean:', nextLineClean);
                          
                          // Check if this is the intro text
                          if (nextLineClean.includes('current sprint') && nextLineClean.includes('tickets')) {
                            introText = nextLineClean;
                            console.log('>>> CAPTURED INTRO TEXT for health assessment:', introText);
                            // Mark this line as consumed
                            consumedIndices.add(i);
                            console.log(`>>> MARKED index ${i} as consumed`);
                            break;
                          }
                      ...
                      const scoreColor = scoreValue >= 80 ? '#10b981' : scoreValue >= 60 ? '#3b82f6' : scoreValue >= 40 ? '#f59e0b' : '#ef4444';
                      const scoreEmoji = scoreValue >= 80 ? '🟢' : scoreValue >= 60 ? '🔵' : scoreValue >= 40 ? '🟡' : '🔴';
                      const scoreRange = scoreValue >= 80 ? 'excellent' : scoreValue >= 60 ? 'good' : scoreValue >= 40 ? 'warning' : 'critical';
                      
                      const statusLabel = scoreValue >= 80 ? 'Excellent' : scoreValue >= 60 ? 'Good' : scoreValue >= 40 ? 'Needs Attention' : 'Critical';
                      const improvement = scoreValue >= 80 ? '+5' : scoreValue >= 60 ? '+12' : scoreValue >= 40 ? '+25' : '+38';
                      const trend = scoreValue >= 60 ? 'up' : 'down';
                      const actionText = scoreValue >= 80 ? 'Maintain momentum' : 
                                       scoreValue >= 60 ? 'Optimize alignment' : 
                                       scoreValue >= 40 ? 'Refocus priorities' : 
                                       'Urgent realignment needed';
                      
                      // Calculate category breakdown (mock data - you'd get this from your analysis)
                      const coreValuePercent = Math.min(100, scoreValue + 20);
                      const strategicEnablerPercent = Math.max(0, scoreValue - 10);
                      const driftPercent = Math.max(0, 60 - scoreValue);
                      const distractionPercent = Math.max(0, 40 - scoreValue);
                      
                      return (
                        <motion.div 
                          key={index} 
                          className="health-assessment-combined glass"
                          data-score-range={scoreRange}
                        ...
                        >
                          <div className="health-glow-effect" />
                          <div className="health-pattern-overlay" />
                          ...
                          <div className="health-content-wrapper">
                            ...
                            <div className="health-score-visual">
                              <div className="score-ring-container">
                                <svg className="score-ring" viewBox="0 0 120 120">
                                  ...
                                  <circle
                                    ...
                                    fill="none"
                                    strokeWidth="12"
                                    strokeDasharray={`${327 * (scoreValue / 100)} 327`}
                              ...
                              <div className="score-status">
                                <span className="status-emoji">{scoreEmoji}</span>
                              ...
                              <div className="health-indicator-bar">
                                <div className="indicator-track">
                                  <div 
                                    ...
                                    style={{
                                      width: `${scoreValue}%`,
                                      background: `linear-gradient(90deg, ${scoreColor}40 0%, ${scoreColor} 100%)`
                                    }}
                                  />
                                  <div 
                                ...
                                <div className="indicator-markers">
                    ...
                    if (trimmed.startsWith('•') || trimmed.startsWith('-')) {
                      ...
                      for (let i = index - 1; i >= 0; i--) {
                        ...
                        if (lineContent.includes('next steps') || lineContent.includes('📋 next steps')) {
                      ...
                      const isNextStep = currentSection === 'nextsteps';
                      
                      if (isRecommendation || isNextStep) {
                        ...
                        return (
                          ...
                          >
                            ...
                            <span className="bullet-text">
                              {(() => {
                                // Parse SCRUM-XX references to make them clickable
                                const parts = cleanText.split(/(SCRUM-\d+)/g);
                                if (parts.some(part => /^SCRUM-\d+$/.test(part))) {
                                  return parts.map((part, partIndex) => {
                                    if (/^SCRUM-\d+$/.test(part)) {
                                      const ticket = result.tickets.find(t => t.key === part);
                                      ...
                                      return (
                                        <span
                                          key={partIndex}
                                          className="inline-ticket-reference"
                                          onClick={(e) => {
                                            e.stopPropagation();
                                            const ticketElement = document.getElementById(`ticket-${part}`);
                                            if (ticketElement) {
                                              ticketElement.scrollIntoView({ behavior: 'smooth', block: 'center' });
                                              ticketElement.classList.add('highlight-pulse');
                                              setTimeout(() => {
                                                ticketElement.classList.remove('highlight-pulse');
                                              }, 2000);
                                            }
                                          }}
                                          style={{
                                            ...
                                            textDecorationColor: `${categoryColor}40`,
                                          ...
                                          onMouseEnter={(e) => {
                                            ...
                                            e.currentTarget.style.textDecorationColor = categoryColor;
                                          }}
                                          onMouseLeave={(e) => {
                                            e.currentTarget.style.backgroundColor = 'transparent';
                                            e.currentTarget.style.textDecorationColor = `${categoryColor}40`;
                                          }}
                                        >
                                          {part}
                                        </span>
                        ...
                        // Extract ticket keys from [[...]] format for the expandable section
                        ...
                        // Parse inline SCRUM-XX references in the text to make them clickable
                        let displayContent;
                        const parts = displayText.split(/(SCRUM-\d+)/g);
                        
                        // Check if we have any SCRUM references to make clickable
                        if (parts.some(part => /^SCRUM-\d+$/.test(part))) {
                          displayContent = (
                            <>
                              {parts.map((part, partIndex) => {
                                if (/^SCRUM-\d+$/.test(part)) {
                                  // This is a ticket reference, make it clickable
                                  const ticket = result.tickets.find(t => t.key === part);
                                  const categoryColor = ticket ? (
                                    ticket.category === 'core_value' ? '#10b981' :
                                    ticket.category === 'strategic_enabler' ? '#3b82f6' :
                          ...
                          // No SCRUM references found, just display the text as is
                                  ...
                                  <div className="ticket-keys-list">
                                    {ticketKeys.map((key, keyIndex) => {
                                      ...
                                      return (
                                        <span 
                                          key={keyIndex}
                                          className="ticket-key-badge clickable"
                                          onClick={(e) => {
                                            e.stopPropagation();
                                            // Scroll to the ticket in the tickets section
                                            const ticketElement = document.getElementById(`ticket-${key}`);
                                            if (ticketElement) {
                                              ticketElement.scrollIntoView({ behavior: 'smooth', block: 'center' });
                    ...
                    // Skip Bottom Line paragraphs
                    const bottomLineCleanParagraph = trimmed.replace(/\*\*/g, '').trim();
                    const bottomLineCheck = bottomLineCleanParagraph.toLowerCase().includes('bottom line') || 
                                         bottomLineCleanParagraph.toLowerCase().startsWith('bottom line:') ||
                                         trimmed.toLowerCase().includes('**bottom line**');
                    if (bottomLineCheck) {
                      console.log('>>> SKIPPING BOTTOM LINE:', trimmed);
                      return null; // Skip bottom line completely
                    }
                    
                    
                    if (isIntro && cleanParagraph.length > 50) {
                      // But NEVER render as intro if it's the text after health assessment
                      if (index > 0) {
                        const lines = result.executiveSummary.split('\n');
                        // Look back up to 2 lines for health assessment
                        for (let i = index - 1; i >= Math.max(0, index - 2); i--) {
                          const prevLine = lines[i].trim().replace(/\*\*/g, '');
                          if (prevLine.includes('Strategic Health Assessment') && 
                              cleanParagraph.includes('current sprint') && 
                              cleanParagraph.includes('tickets')) {
                            console.log(`>>> NOT RENDERING AS INTRO - this is the health assessment description`);
                            return null;
                          }
                        }
                      }
                      console.log(`>>> RENDERING INTRO at index ${index}, isIntro=${isIntro}, consumed=${consumedIndices.has(index)}:`, cleanParagraph);
                      return (
                        <motion.div 
                          key={index} 
                    ...
                    // Regular paragraphs - but NEVER render if it's the intro text after health assessment
                    if (index > 0) {
                      const prevLine = result.executiveSummary.split('\n')[index - 1].trim().replace(/\*\*/g, '');
                      if (prevLine.includes('Strategic Health Assessment') && 
                          cleanParagraph.includes('current sprint') && 
                          cleanParagraph.includes('tickets')) {
                        console.log(`>>> DEFINITELY SKIPPING index ${index} - this should NOT be rendered as regular paragraph!`);
                        return null;
                      }
                    }
                    
                    console.log(`>>> RENDERING REGULAR PARAGRAPH at index ${index}:`, cleanParagraph);
